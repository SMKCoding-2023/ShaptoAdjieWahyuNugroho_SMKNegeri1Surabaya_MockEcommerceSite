...
import type { Product, FormField } from '~/types/product';
...
const formFormat = [
    ...
    { name: "image", label: "Image", type: "file", value: {}, placeholder: "" },
...
productStore.getProductList().then(() => {
    ...
    for(let i = 0; i < formFormat.length; i++){
        formTable.value.push(formFormat[i]);
        formTable.value[i].value = product.value[formFormat[i].name];
    }
...
const updateProduct = async () => {
    ...
    let file = formTable.value.find((item) => item.type === "file");
    
    if(file.value.constructor === Object) {
    ...
    formTable.value.forEach((item: FormField) =>
    {
        if (item.name)
        {
    ...
    result.image = file;
    await productStore.updateProduct(id, result);
...
<template>
    <section>
        <div v-if="isSiteLoading">
            <h1>Loading product, please wait...</h1>
        </div>
        <div v-else>
            <h1>Edit Product</h1>
            ...
            <form @submit.prevent="updateProduct">
                <div class="formEntry" v-for="(item, index) in formTable" :key="index">
                ...
                <button type="submit" class="submit">
                    <span v-if="!isFormProcessing">Submit</span>
