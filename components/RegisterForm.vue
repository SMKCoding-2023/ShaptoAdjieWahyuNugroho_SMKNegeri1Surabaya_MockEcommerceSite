    ...
    <div>
        <form @submit.prevent="submitRegister">
            <div class="headingContainer">
                <h5 class="registerTitle">Register</h5>
                <span class="registerHeading">Create an account to start using our application</span>
            </div>
            <div v-if="isShowAlert" :class="isSuccess ? 'success' : 'fail'" role="alert">
                {{ message }}
            </div>
            <div v-for="(item, index) in register" :key="index" class="formItem">
...
template > div {
    ...
    @media (min-width: 768px) { /* Apply padding 8 at medium screens and up */
        padding: 2rem;
    }
}
